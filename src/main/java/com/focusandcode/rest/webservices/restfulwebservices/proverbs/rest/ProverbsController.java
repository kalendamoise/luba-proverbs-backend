package com.focusandcode.rest.webservices.restfulwebservices.proverbs.rest;
...
import com.focusandcode.rest.webservices.restfulwebservices.proverbs.services.CategoryDaoService;
import com.focusandcode.rest.webservices.restfulwebservices.proverbs.exceptions.CategoryNotFoundException;
import com.focusandcode.rest.webservices.restfulwebservices.proverbs.services.ProverbDaoService;
...
public class ProverbsController {
    ...
    public CollectionModel<EntityModel<Proverb>> retrieveAllProverbs() {
        ...
        List<EntityModel<Proverb>> myProverbs = StreamSupport.stream(proverbs.spliterator(), false)
                .map(proverb -> {
                    return new EntityModel<Proverb>(proverb,
                            linkTo(methodOn(ProverbsController.class).retrieveProverb(proverb.getId())).withSelfRel());
    ...
    public ResponseEntity<Object> createProverb(@Valid @RequestBody Proverb proverb) {
        ...
        URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(newProverb.getId()).toUri();
        ...
        List<EntityModel<Category>> myCategories = StreamSupport.stream(categories.spliterator(), false)
                .map(category -> {
                    return new EntityModel<Category>(category,
                            linkTo(methodOn(ProverbsController.class).retrieveCategory(category.getId())).withSelfRel());
                }).collect(Collectors.toList());
    ...
    public ResponseEntity<Object> createCategory(@Valid @RequestBody Category category) {
        ...
        URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(newCategory.getId()).toUri();
        return ResponseEntity.created(location).build();
    }
}
